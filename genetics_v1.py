...
class GA(object):
    ...
    def __init__(self, train, valid, estimator, feval, 
                 ...
                 verbose=False):
        self.data = np.concatenate((train, valid), axis = 0)
        self.train = train
    ...
    def _sample_adapt_score(self, gene):
        ...
        #yprob = estor.predict_proba(sample_v)
        #return self.feval(estor, predicts, yprob, self.valid.T[-1].T, gene)
    ...
    def _select_feature_adapt_score(self, gene):
        ...
        #yprob = estor.predict_proba(valid_fs)
        #return self.feval(estor, predicts, yprob, self.valid.T[-1].T, gene)
        return self.feval(predicts, self.valid.T[-1].T)
        #return np.mean(abs(predicts - self.valid.T[-1].T) / self.valid.T[-1].T)
    ...
    def _run(self, n_gene_units, n_sample, adapt_func):
        ...
        for i in range(self.iter):
            if(self.do_shuffle):
                rand_idx = np.arange(self.data.shape[0])
                np.random.shuffle(rand_idx)
                self.train = self.data[rand_idx[:n_sample]]
                self.valid = self.data[rand_idx[n_sample:]]
            
 
    # n_gene_units: how many units in one gene?
    def old_run(self, n_gene_units, n_sample, adapt_func):
        if n_gene_units == n_sample:
            return self.train
